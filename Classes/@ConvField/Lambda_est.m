function Lambda = Lambda_est( cfield, dcfield )
% LAMBDA_EST( cfield, grad_cfield ) estimates the induced Riemannian metric
% from a convolution field.
% The Riemannian metric is known as the Lambda matrix in neuroimaging.
...
%--------------------------------------------------------------------------
% OUTPUT
%   Lambda  an object of class Field representing the Riemannian metric
...
if( dcfield.fibersize(1) ~= cfield.fibersize(end) )
...
Lambda       = Field( cfield.mask );
% Insert xvals values into the Field and initialise the field property with
% the correct dimensions
Lambda.xvals = cfield.xvals;
Lambda.field = zeros( [ cfield.masksize D D ] );
...
switch D
    case 1
        ...
        Lambda.field( index{:}, 1 ) = ( VdxY .* VY - CYdxY.^2 ) ./ VY.^2;
    ...
    case 2
        ...
        % Prepare output
        Lambda.field( index{:}, 1, 1 ) = g_xx;
        Lambda.field( index{:}, 1, 2 ) = g_xy;
        Lambda.field( index{:}, 2, 1 ) = g_xy;
        Lambda.field( index{:}, 2, 2 ) = g_yy;
    ...
    case 3
        ...
        Lambda.field( index{:}, 1, 1 ) = g_xx;
        Lambda.field( index{:}, 1, 2 ) = g_xy;
        Lambda.field( index{:}, 1, 3 ) = g_xz;
        Lambda.field( index{:}, 2, 1 ) = g_xy;
        Lambda.field( index{:}, 2, 2 ) = g_yy;
        Lambda.field( index{:}, 2, 3 ) = g_yz;
        Lambda.field( index{:}, 3, 1 ) = g_xz;
        Lambda.field( index{:}, 3, 2 ) = g_yz;
        Lambda.field( index{:}, 3, 3 ) = g_zz;
        
end
%%%%%% END compute the Riemannian metric

% Remove NaNs and replace with zero
Lambda.field = nan2zero( Lambda.field );
