...
% VAR( varargin ) does take the mean of specified fiber dimensions.
%--------------------------------------------------------------------------
% ARGUMENTS
% varargin
%  If 1: inputs are
...
%--------------------------------------------------------------------------
% OUTPUT
% obj  an object of class Field where the variance of the specified fiber
%      is computed. If nargin==1, then the last fiberdimension is taken the
%      variance of.
%
%--------------------------------------------------------------------------
% EXAMPLES
...
if nargin == 1
   if isa( varargin{1}, "Field")
       out = varargin{1};
       out.field = var( out.field, 0, out.D + out.fiberD );
   end
elseif nargin == 2
   if isa( varargin{1}, "Field")
       D = varargin{2};
       if isnumeric( D )
          if D > 0
            out = varargin{1};
            out.field = var( out.field, 0, out.D + D );
          else
