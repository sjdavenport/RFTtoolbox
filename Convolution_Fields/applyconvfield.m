function [field_vals] = applyconvfield(tval, Y, Kernel, xvals_vecs, truncation)
...
if isnumeric(Kernel)
    ...
    if truncation == 1
        error('Truncation has not been implemented for non-Gaussian kernels')
    end
end
...
if truncation == 0
    ...
    trunwindow = zeros(1,D);
    eval_vecs = cell(1,D);
    for d = 1:D
        trunwindow(d) = min([round(4*sigma) length(xvals_vecs{d})/2]);
        eval_vecs{d} = -trunwindow(d):trunwindow(d);
    end
end
if D > 3
