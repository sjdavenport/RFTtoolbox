 function [ peaklocs, peakvals ] = findlms_mod( fn, initial_estimates, lowerbounds, upperbounds )
...
for peakI = 1:npeaks
    lb = lowerbounds(:,peakI);
    ub = upperbounds(:,peakI);
    
    % Use the optimization routine fmincon to find the peaks
    peaklocs(:,peakI) = fmincon(@(tval) -fn(tval), initial_estimates(:,peakI), [], [], [], [], lb, ub, mfield, options);
