function obj = wtfield( varmask, fibersize, df, xvals )
% wtfield( varmask, fibersize, df, xvals ) constructs a Fields object having
% white t noise in the fiber.
%--------------------------------------------------------------------------
% ARGUMENTS
% Mandatory
...
if islogical( varmask)
    obj = Field( varmask );
    obj.field = trnd( df, [ obj.masksize(1:obj.D) fibersize ] );    
else
    obj = Field( varmask );
    obj.field = trnd( df, [ obj.masksize(1:obj.D) fibersize ] );
end
